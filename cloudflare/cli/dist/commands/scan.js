...
function collectFiles(dir, exts, out, maxDepth, depth = 0) {
    if (depth > maxDepth)
        return;
    try {
...
function fileToRoute(file, srcDir) {
    ...
    if (staticMap[last])
        return staticMap[last];
    // Nest segments if multiple depth
    return '/' + stripped.join('/');
}
// ── JSX text extraction ────────────────────────────────────────────────────────
function extractPageContent(source, route, filePath) {
    ...
    for (const t of propTexts) {
        if (t.length > 40 && !paragraphs.includes(t))
            paragraphs.push(t);
        else if (t.length <= 40 && !headings.includes(t))
            headings.push(t);
    }
...
function extractTagText(source, tagPattern, minLen, maxLen) {
    const results = [];
...
function extractLinkText(source) {
    ...
    while ((m = re.exec(source)) !== null) {
        const text = m[1].trim().replace(/\s+/g, ' ');
        if (isUsableText(text) && !text.startsWith('/') && !text.startsWith('http'))
            results.push(text);
    }
    return [...new Set(results)];
}
function extractPropValues(source, propNames) {
    const results = [];
 ...
 */
function extractPricingTiers(source) {
    const tiers = [];
    ...
    while ((m = planNameRe.exec(source)) !== null) {
        ...
        priceRe.lastIndex = 0;
        if (!priceMatch && name !== 'Enterprise' && name !== 'Free')
            continue;
...
function buildContentJson(pages, brand, siteUrl) {
    ...
    for (const page of pages) {
        ...
        if (page.headings.length > 0) {
            ...
            if (page.headings[1])
                heroSection.subheading = page.headings[1];
            if (page.paragraphs[0])
                heroSection.text = page.paragraphs[0];
            if (page.ctaButtons[0])
                heroSection.cta = { label: page.ctaButtons[0], href: page.route === '/' ? (siteUrl ? `${siteUrl}/signup` : '/signup') : page.route };
            sections.push(heroSection);
        }
        // Features/items section from list items
        if (page.listItems.length >= 2 && page.pricingTiers.length === 0) {
            const featHeading = page.headings.find((h) => /feature|benefit|offer|include|why|what|how|capabilit/i.test(h));
            sections.push({
                type: 'features',
        ...
        // CTA section
        const ctaHeading = page.headings.find((h) => /ready|start|get started|sign up|try|join|begin/i.test(h));
        if (ctaHeading) {
            sections.push({
                type: 'cta',
                heading: ctaHeading,
...
// ── Print summary ─────────────────────────────────────────────────────────────
function printSummary(pages, contentJson) {
    const json = contentJson;
    const count = Object.keys(json.pages ?? {}).length;
    console.log(`\n  ${chalk.bold('Extracted content for:')} ${count} pages\n`);
    for (const page of pages) {
        const sections = json.pages?.[page.route]?.sections?.length ?? 0;
        console.log(`    ${chalk.green('✓')} ${chalk.cyan(page.route.padEnd(20))}  ` +

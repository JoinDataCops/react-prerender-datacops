...
import { listSbProjects, getSbApiKeys, executeSbSqlFile, setSbSecrets, } from '../lib/supabase-api.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
function findPostgresSchema() {
    const candidates = [
...
function findEdgeFunctionTemplates() {
    const candidates = [
...
function findSupabaseMiddlewareTemplate() {
    const candidates = [
...
export async function initSupabase(opts) {
    ...
    if (tplDir) {
        ...
        if (scaffoldEdge) {
            ...
            try {
                ...
                console.log(chalk.yellow(`  Could not scaffold: ${e.message}`));
            }
        }
    }
    // Scaffold middleware
...
async function deployWithSupabaseCli(ref) {
    const functions = [
        'prerender',
    ...
    for (const fn of functions) {
        ...
        const result = spawnSync('supabase', ['functions', 'deploy', fn, '--project-ref', ref], { shell: true, encoding: 'utf8', cwd: process.cwd() });
        if (result.status === 0) {
...
//# sourceMappingURL=init-supabase.js.map
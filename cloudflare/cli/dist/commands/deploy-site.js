...
import { getApiToken, getProject, getSiteConfig, setSiteConfig, } from '../lib/config.js';
import { listAccounts, listPagesProjects, createPagesProject, updatePagesEnvVars, } from '../lib/cf-api.js';
import { detectFramework, getUnknownFramework, SPA_FRAMEWORKS, } from '../lib/detect-framework.js';
export async function deploySiteCommand(opts = {}) {
...
async function collectSiteConfig(token, project) {
    console.log(chalk.bold.cyan('\n  Site Deployment Setup\n'));
    console.log(chalk.dim('  prerender-edge is designed for pure SPAs (React, Vue, Angular, etc.)\n') +
    ...
    const profile = SPA_FRAMEWORKS.find((f) => f.name === frameworkChoice) ?? getUnknownFramework();
    // Build command
...
// ── Build ─────────────────────────────────────────────────────────────────────
async function runBuild(buildCommand, outputDir) {
...
async function ensurePagesProject(token, projectName) {
    ...
    const r = spawnSync('npx', ['wrangler', 'pages', 'project', 'create', projectName, '--production-branch', 'main'], { shell: true, encoding: 'utf8', stdio: 'pipe' });
    if (r.status === 0) {
...
async function runPagesDeploy(outputDir, projectName) {
    const s = ora('Uploading to Cloudflare Pages...').start();
    const result = spawnSync('npx', ['wrangler', 'pages', 'deploy', outputDir, '--project-name', projectName, '--commit-dirty=true'], {
        cwd: process.cwd(),
        shell: true,
        encoding: 'utf8',
        stdio: 'pipe',
    ...
    const isAuthError = output.includes('Authentication error') ||
    ...
    if (isAuthError) {
        ...
        output.split('\n').filter((l) => l.trim()).slice(-15).forEach((l) => console.log(chalk.dim(`    ${l.trim()}`)));
        console.log('\n  To deploy manually:');
...
async function setPageEnvVars(token, accountId, projectName, workerUrl, workerSecret) {
    if (!accountId)
        return;

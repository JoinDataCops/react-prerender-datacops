...
async function workerFetch(url, secret, path, options = {}) {
...
export async function cacheRefreshCommand(opts) {
    ...
    try {
        const result = await workerFetch(url, secret, '/api/cache/generate', {
            method: 'POST',
            body: JSON.stringify({ force: opts.force ?? false }),
        });
        if (result.success) {
            spinner.succeed(`Cache refreshed — ${chalk.bold(result.pages_synced)} pages in ${result.time_ms}ms`);
        }
        else {
...
export async function cacheClearCommand(opts) {
    ...
    if (opts.all || (!opts.path)) {
        ...
        let total = 0;
        try {
            const stats = await workerFetch(url, secret, '/api/prerender/stats');
            total = stats.total;
        }
        catch {
            // ignore
        ...
        try {
            const result = await workerFetch(url, secret, '/api/prerender?all=true', { method: 'DELETE' });
    ...
    try {
        const result = await workerFetch(url, secret, `/api/prerender?path=${encodeURIComponent(targetPath)}`, { method: 'DELETE' });
        if (result.success) {
...
export async function cacheStatsCommand() {
    ...
    try {
        const [stats, cronRuns] = await Promise.all([
            workerFetch(url, secret, '/api/prerender/stats'),
            workerFetch(url, secret, '/api/cron/runs?limit=5'),
        ]);
        spinner.stop();
        ...
        if (cronRuns.runs.length > 0) {
            ...
            for (const run of cronRuns.runs) {
                ...
                const duration = run.completed_at
                    ...
                    : 'running...';
                console.log(`  ${statusColor(run.status.padEnd(10))} ${run.started_at.replace('T', ' ').slice(0, 19)}  ${run.pages_synced} synced  ${duration}${run.error_message ? '  ' + chalk.red(run.error_message.slice(0, 50)) : ''}`);
            }
        }
        console.log();
    }

...
export async function runPageWizard(
  siteUrl: string,
  _workerDir: string,
): Promise<PageWizardResult> {
  const base = siteUrl.replace(/\/$/, '');
  const hostname = (() => { try { return new URL(base).hostname.replace(/^www\./, ''); } catch { return base; } })();

  console.log(chalk.bold.cyan('\n  Page Content Setup\n'));
  console.log(
    chalk.dim('  Configure what Googlebot, Twitterbot, and other crawlers see.\n') +
...
async function collectPage(
  ...
  while (true) {
    ...
    if (bullet.toLowerCase() === 'skip') { break; }
    if (bullet === '') {
      emptyCount++;
...
function buildSchemas(path: string, title: string, description: string, siteUrl: string): object[] {
 ...
 */
export async function writePageConfigsToD1(
  token: string,
  accountId: string,
  dbId: string,
  result: PageWizardResult,

...
export async function statusCommand(): Promise<void> {
  ...
  try {
    ...
    const db = databases.find(
      (d) => d.name === project.dbName || d.uuid === project.dbId,
    );
    ...
    if (project.workerUrl && project.workerSecret) {
      ...
      try {
        ...
        if (cronRes.ok) {
          const data = await cronRes.json() as {
            runs: { status: string; started_at: string; pages_synced: number; error_message: string | null }[];
          };
          if (data.runs.length > 0) {
            ...
            for (const run of data.runs) {
              ...
              console.log(
                `    ${statusIcon} ${run.started_at.split('T')[0]}  ${run.status.padEnd(10)}  ${run.pages_synced} pages synced${run.error_message ? '  ' + chalk.red(run.error_message.slice(0, 40)) : ''}`,
              );
            }
          }
        }

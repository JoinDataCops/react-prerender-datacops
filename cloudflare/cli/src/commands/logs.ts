...
export async function logsCommand(opts: { worker?: string }): Promise<void> {
  ...
  try {
    ...
    ws.addEventListener('message', (event) => {
      try {
        const data = JSON.parse(event.data as string) as TailEvent;
        printTailEvent(data);
      } catch {
...
function printTailEvent(event: TailEvent): void {
  ...
  if (event.event?.request) {
    ...
    console.log(
      `${chalk.dim(ts)}  ${outcomeColor(event.outcome.padEnd(10))}  ${chalk.cyan(method.padEnd(6))} ${path}`,
    );
  } else if (event.event?.cron) {
    console.log(
      `${chalk.dim(ts)}  ${outcomeColor(event.outcome.padEnd(10))}  ${chalk.magenta('cron')}  ${event.event.cron}`,
    );
  }

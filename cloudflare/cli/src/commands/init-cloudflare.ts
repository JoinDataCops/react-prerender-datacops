...
import {
  ...
  getWorkerSubdomain, listWorkers, getPagesProject,
  CloudflareApiError,
} from '../lib/cf-api.js';
...
function findWorkerTemplate(): string | null {
  const candidates = [
...
function findMiddlewareTemplate(): string | null {
  const candidates = [
...
async function checkStatus(
  ...
  const workerDeployed = workers.some(
    (w) => w.id === project.workerName || w.id?.includes(project.workerName),
  );
...
function isComplete(s: SetupStatus, project: NonNullable<ReturnType<typeof getProject>>): boolean {
  return (
    s.dbExists &&
    s.schemaApplied &&
    s.workerFilesExist &&
...
export async function initCloudflare(opts: { force?: boolean }): Promise<void> {
  ...
  if (existing && !opts.force) {
    ...
    if (complete) {
      ...
      if (action === 'redeploy') { await deployCommand({ workerDir }); return; }
      if (action === 'secrets') { await pushSecrets(token, existing, workerDir); return; }
  ...
  let accountId: string, accountName: string;
  ...
  try { pagesProjects = await listPagesProjects(token, accountId); } catch {}
  pagesSpin.stop();
...
function getMissingSteps(s: SetupStatus, project: NonNullable<ReturnType<typeof getProject>>): string[] {
  const missing: string[] = [];
  if (!s.dbExists) missing.push('create DB');
  if (!s.schemaApplied) missing.push('apply schema');
  if (!s.workerFilesExist) missing.push('scaffold files');
  if (!s.workerDeployed) missing.push('deploy worker');
  if (project.pagesProject && !s.pagesEnvSet) missing.push('set Pages env vars');
  if (!s.middlewareExists) missing.push('scaffold middleware');
  return missing;
}

async function runMissingSteps(
  token: string,
  project: NonNullable<ReturnType<typeof getProject>>,
  status: SetupStatus,
  workerDir: string,
  ...
  if (!status.dbExists) {
    ...
    } catch (err) { s.fail('Failed to create DB'); throw err; }
  }
...
async function pushSecrets(
  ...
  const cmd = (secret: string, value: string) =>
    spawnSync('npx', ['wrangler', 'secret', 'put', secret, '--env='], {
      cwd: workerDir,
      input: value + '\n',
      encoding: 'utf8',
      shell: true,
...
async function scaffoldWorker(
  ...
  try {
    ...
    console.log(chalk.yellow(`  Could not scaffold worker: ${(e as Error).message}`));
  }
}

function scaffoldMiddleware(): void {
...
async function setPagesEnvVars(
  token: string,
  accountId: string,
  pagesProject: string,
  workerUrl: string,
  ...
  try {
    ...
    const isAuthError =
      msg.includes('10000') || msg.includes('Authentication') ||
    ...
    if (!isNotFound) {
      ...
      const r1 = spawnSync(
        'npx wrangler',
        ['pages', 'env', 'put', 'WORKER_URL', '--project-name', pagesProject, '--env', 'production'],
        { input: workerUrl + '\n', shell: true, encoding: 'utf8', stdio: 'pipe' },
      );
      const r2 = spawnSync(
        'npx wrangler',
        ['pages', 'env', 'put', 'WORKER_SECRET', '--project-name', pagesProject, '--env', 'production'],
        { input: workerSecret + '\n', shell: true, encoding: 'utf8', stdio: 'pipe' },
      );
      if (r1.status === 0 && r2.status === 0) {
...
async function triggerInitialCache(
  ...
  try {
    ...
    if (resp.ok) {
      const result = await resp.json() as { success: boolean; pages_synced: number; time_ms: number };
      if (result.success) {

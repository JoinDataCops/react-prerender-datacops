...
function findSchema(type: 'd1' | 'postgres'): string {
...
async function migrateCloudflare(reset?: boolean): Promise<void> {
  ...
  if (!accountId) { console.log(chalk.red('  No account found.')); process.exit(1); }
  ...
  } catch (err) { s.fail('Migration failed'); throw err; }
}

async function migrateSupabase(reset?: boolean): Promise<void> {
